...
const addMonths = (date, monthsToAdd) => {
  ...
  next.setMonth(next.getMonth() + monthsToAdd, 1);
  return next;
};
...
const CampervanSchedule = () => {
  ...
  const [scheduleChartWidth, setScheduleChartWidth] = useState(0);
  ...
  const firstEditableMonthIndex = useMemo(() => {
    const firstEligibleRow = rows.find((row) => {
      const status = String(row.regentProduction || '').trim().toLowerCase();
      const isBlocked = status === 'production commenced regent';
      const hasDate = Boolean(parseDateValue(row.forecastProductionDate));
      return !isBlocked && hasDate;
    });
    if (!firstEligibleRow) return 0;
  ...
  useEffect(() => {
    ...
    const secondPointMonth = new Date(2026, 5, 1);
    const secondIndex = scheduleMonths.findIndex(
      (month) =>
        month.getFullYear() === secondPointMonth.getFullYear() &&
        month.getMonth() === secondPointMonth.getMonth(),
    );
    ...
    setSchedulePoints([
      { id: 1, monthIndex: firstEditableMonthIndex, value: 1 },
      { id: 2, monthIndex: resolvedSecondIndex, value: 2 },
    ]);
    scheduleInitializedRef.current = true;
  }, [firstEditableMonthIndex, scheduleMonths]);
  ...
  useEffect(() => {
    ...
    setSchedulePoints((prev) => {
      ...
      const updated = sorted.map((point, index) =>
        index === 0 ? { ...point, monthIndex: newIndex } : point,
      );
      return updated;
    });
  }, [firstEditableMonthIndex, scheduleMonths.length]);

  useEffect(() => {
    if (!scheduleChartRef.current) return;
    const observer = new ResizeObserver((entries) => {
      entries.forEach((entry) => {
        setScheduleChartWidth(entry.contentRect.width);
      });
    });
    observer.observe(scheduleChartRef.current);
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Campervan Schedule">
  ...
  const sortedSchedulePoints = useMemo(() => {
    return [...schedulePoints].sort((a, b) => a.monthIndex - b.monthIndex);
  }, [schedulePoints]);
  ...
  const getScheduleY = (value) => {
    const { margin, plotHeight } = scheduleChartMetrics;
    const maxValue = 5;
    const minValue = 0;
  ...
  const scheduleStepSegments = useMemo(() => {
    if (sortedSchedulePoints.length === 0) return [];
    const segments = [];
    for (let i = 0; i < sortedSchedulePoints.length - 1; i += 1) {
      const current = sortedSchedulePoints[i];
      const next = sortedSchedulePoints[i + 1];
      segments.push({
        x1: getScheduleX(current.monthIndex),
        y1: getScheduleY(current.value),
        x2: getScheduleX(next.monthIndex),
        y2: getScheduleY(current.value),
      });
      segments.push({
        x1: getScheduleX(next.monthIndex),
        y1: getScheduleY(current.value),
        x2: getScheduleX(next.monthIndex),
  ...
  const snapScheduleValue = (y) => {
    ...
    const ratio = (y - margin.top) / plotHeight;
    const rawValue = 5 - ratio * 5;
    const snapped = Math.round(rawValue);
    return Math.min(Math.max(snapped, 1), 5);
  };

  useEffect(() => {
    if (!draggingPointId && !draggingLine) return;
    const handleMove = (event) => {
      ...
      if (draggingPointId) {
        ...
        setSchedulePoints((prev) => {
          const sorted = [...prev].sort((a, b) => a.monthIndex - b.monthIndex);
          const index = sorted.findIndex((point) => point.id === draggingPointId);
          ...
          const updated = sorted.map((point) =>
            point.id === draggingPointId
              ? { ...point, monthIndex: newMonthIndex, value: newValue }
              : point,
          );
          return updated;
        });
        ...
        setPendingLinePoint({
          monthIndex: snapScheduleMonthIndex(x),
          value: snapScheduleValue(y),
        });
      }
    };

    const handleUp = () => {
      if (draggingLine && pendingLinePoint) {
        scheduleDirtyRef.current = true;
        setSchedulePoints((prev) => {
          const sorted = [...prev].sort((a, b) => a.monthIndex - b.monthIndex);
          if (sorted.some((point) => point.monthIndex === pendingLinePoint.monthIndex)) {
            return prev;
          }
          const insertIndex = sorted.findIndex(
            (point) => point.monthIndex > pendingLinePoint.monthIndex,
          );
          ...
          if (sorted.some((point) => point.monthIndex === clampedIndex)) return prev;
          const newPoint = {
            id: Date.now() + Math.random(),
            monthIndex: clampedIndex,
            value: pendingLinePoint.value,
          };
          if (insertIndex === -1) return [...sorted, newPoint];
          return [...sorted.slice(0, insertIndex), newPoint, ...sorted.slice(insertIndex)];
        });
      }
    ...
    draggingLine,
    pendingLinePoint,
    scheduleChartMetrics,
    scheduleMonths.length,
    firstEditableMonthIndex,
  ]);
          ...
          <div className="mt-6 border-t border-gray-100 pt-6">
            ...
            <div className="mt-6 rounded-xl border border-gray-100 bg-white p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                >
                  {deleteMode ? 'Click a point to delete' : 'Delete point'}
                </button>
              </div>
              <div className="mt-4 overflow-x-auto" ref={scheduleChartRef}>
                <svg
                  width={scheduleChartMetrics.width || 600}
                  height={scheduleChartMetrics.height}
                  className="min-w-[600px]"
                >
                  <rect
                    x={scheduleChartMetrics.margin.left}
                    y={scheduleChartMetrics.margin.top}
                    width={scheduleChartMetrics.plotWidth}
                    height={scheduleChartMetrics.plotHeight}
                    fill="#f8fafc"
                    rx={12}
                  />
                  {sortedSchedulePoints.length > 0 && (
                    <rect
                      x={scheduleChartMetrics.margin.left}
                      y={scheduleChartMetrics.margin.top}
                      width={
                        getScheduleX(sortedSchedulePoints[0].monthIndex) -
                        scheduleChartMetrics.margin.left
                      }
                      height={scheduleChartMetrics.plotHeight}
                      fill="#e2e8f0"
                      opacity={0.6}
                  ...
                  {sortedSchedulePoints.length > 0 && (
                    <text
                      ...
                      y={scheduleChartMetrics.margin.top + 18}
                      textAnchor="middle"
                      fontSize={11}
                      fill="#64748b"
                    >
                  ...
                  {Array.from({ length: 6 }).map((_, index) => {
                    const value = 5 - index;
                    const y = getScheduleY(value);
                    return (
                      <g key={`y-${value}`}>
                        <line
                          x1={scheduleChartMetrics.margin.left}
                          y1={y}
                          x2={scheduleChartMetrics.margin.left + scheduleChartMetrics.plotWidth}
                          y2={y}
                          stroke="#e2e8f0"
                          strokeDasharray="3 3"
                        />
                        <text
                          x={scheduleChartMetrics.margin.left - 10}
                          y={y + 4}
                          textAnchor="end"
                          fontSize={11}
                          fill="#64748b"
                  ...
                  {sortedSchedulePoints.length > 0 && (
                    <line
                      x1={scheduleChartMetrics.margin.left}
                      y1={getScheduleY(0)}
                      x2={getScheduleX(sortedSchedulePoints[0].monthIndex)}
                      y2={getScheduleY(0)}
                      stroke="#94a3b8"
                      strokeDasharray="6 4"
                      strokeWidth={2}
                  ...
                  {scheduleStepSegments.map((segment, index) => (
                    <line
                      key={`segment-${index}`}
                      x1={segment.x1}
                      y1={segment.y1}
                  ...
                  {sortedSchedulePoints.length > 0 && (
                    <path
                      d={`M ${getScheduleX(sortedSchedulePoints[0].monthIndex)} ${getScheduleY(
                        sortedSchedulePoints[0].value,
                      )} ${
                        scheduleStepSegments
                          .map((segment) => `L ${segment.x2} ${segment.y2}`)
                          .join(' ')
                      }`}
                      fill="none"
                      stroke="transparent"
                      ...
                      style={{ cursor: deleteMode ? 'not-allowed' : 'ns-resize' }}
                    />
                  )}
                  {pendingLinePoint && !deleteMode && (
                    <circle
                      cx={getScheduleX(pendingLinePoint.monthIndex)}
                      cy={getScheduleY(pendingLinePoint.value)}
                      r={6}
                      fill="#fbbf24"
                      stroke="#f59e0b"
                      strokeWidth={2}
                  ...
                  {sortedSchedulePoints.map((point) => {
                    const isDeletable = deleteMode;
                    return (
                      <circle
                        key={point.id}
                        cx={getScheduleX(point.monthIndex)}
                        cy={getScheduleY(point.value)}
                        r={7}
                        fill={isDeletable ? '#fb7185' : '#ffffff'}
                        stroke={isDeletable ? '#e11d48' : '#4f46e5'}
                        strokeWidth={2}
                        onMouseDown={(event) => {
