...
const canSendEmail = (config, role) => (
  config?.serviceId && config?.publicKey && getTemplateId(config, role) && config?.recipients?.[role]
);
...
const sendWorkflowEmail = async (role, transfer, config) => {
  ...
  const content = buildEmailHtml(role, transfer, emailTitle, emailNote);
  await emailjs.send(
    config.serviceId,
