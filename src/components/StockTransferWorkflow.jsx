...
const normalizeWorkflowLocation = (value) => {
  ...
  if (normalized === 'perth' || normalized === 'st james') return 'st_james';
...
const getTaskList = (role, transfers, locationKey = '') => buildRows(transfers).filter((transfer) => {
  ...
  if (role === 'location') {
    ...
    return workflow.ceoApprovedAt
      && !workflow.locationDmsDoneAt
      && (!locationKey || transferLocationKey === locationKey);
  }
  if (role === 'planning') return false;
  if (role === 'finance') return isExternalTransfer(transfer) && workflow.locationDmsDoneAt && !workflow.financeDoneAt;
  if (role === 'transport') {
    if (!workflow.locationDmsDoneAt) return false;
...
const getNextEmailRole = (completedRole, transfer) => {
  if (completedRole === 'ceo') return 'location';
...
const TaskCard = ({ role, transfer, onComplete }) => {
  ...
  const actionText = {
    ceo: 'Approve',
    location: isExternalTransfer(transfer) ? 'Confirm DMS return done' : 'Confirm DMS transfer done',
...
const StockTransferWorkflow = ({ role = 'ceo', standalone = false }) => {
  ...
  useEffect(() => {
    buildRows(transfers).forEach(async (transfer) => {
      const workflow = getWorkflow(transfer);
      if (workflow.ceoEmailSentAt || !hasSalesOrder(transfer) || !canSendEmail(config, 'ceo') || ceoEmailSendLocks.current.has(transfer.id)) return;
      ceoEmailSendLocks.current.add(transfer.id);
      try {
        await sendWorkflowEmail('ceo', transfer, config, getTaskList('ceo', transfers).length);
        ...
        ceoEmailSendLocks.current.delete(transfer.id);
  ...
  const completeTask = async (transfer, extra = {}) => {
    ...
    if (role === 'location') {
      updates.workflow = {
        ...
        locationDmsStatus: isExternalTransfer(transfer) ? 'DMS return done' : 'DMS transfer done',
    ...
    try {
      ...
      if (nextRole && canSendEmail(config, nextRole, updatedTransfer)) {
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6">
      ...
      {role === 'location' && (
        <div className="mb-4 rounded-2xl border border-blue-100 bg-white p-4 text-sm text-gray-600 shadow-sm">
          Internal stock transfer: confirm the DMS transfer is completed. External stock transfer: confirm the DMS return is completed.
        </div>
      )}
      {role === 'settings' && (
