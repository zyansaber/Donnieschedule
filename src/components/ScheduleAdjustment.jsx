...
const ScheduleAdjustment = ({ data, shuffleRequests, setShuffleRequests }) => {
  ...
  const sendRequestsEmail = async () => {
    if (!shuffleRequests.length) return;

    const emailRows = shuffleRequests.map((item) => (
      `${item.chassis || ''} | ${subtractDays(item.adjustedTime, 20)} | ${item.monthVin || ''}`
    ));

    const templateParams = {
      title: 'Schedule Shuffling Requests',
      total_count: shuffleRequests.length,
      request_table: emailRows.join('\n'),
    ...
    try {
      ...
      console.error('Failed to send schedule shuffling email:', error);
          ...
          <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
            ...
            <div className="text-sm text-gray-600 mb-3">
              {selectedRows.length > 0
                ? 'Using selected rows only'
                : 'No row selected: showing all dealers'}
            </div>
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="bg-gray-800 text-white px-3 py-2 rounded hover:bg-black" onClick={sendRequestsEmail}>
              Send Email
            </button>
          </div>
        </div>
            ...
            <tbody>
              {shuffleRequests.map((row) => (
                <tr
                  ...
                  className={`cursor-pointer transition-colors ${selectedRows.includes(row.id) ? 'bg-indigo-50' : 'hover:bg-gray-50 active:bg-gray-100'}`}
                >
